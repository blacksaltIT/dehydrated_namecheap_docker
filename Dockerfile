...
CMD [ "/bin/bash", "-c", "(mkdir -pv $DHV_BASEDIR $DHNV_DEPLOYED_CERTDIR $DHNV_DEPLOYED_KEYDIR; ((source <(/app/dehydrated --env); test -f $ACCOUNT_KEY) || /app/dehydrated --register --accept-terms) && /app/dehydrated --cron) & trap \"kill $!\" INT TERM; wait" ]
